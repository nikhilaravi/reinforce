...
var values = underscore$1.values;

var shuffle = underscore$1.shuffle;
var debounce = underscore$1.debounce;
...
var helpers = {
  ...
  decodeFloat: function decodeFloat(x, y, z, w) {
  ...
  sampleArray: function sampleArray(arr) {
    return arr[Math.round(Math.random() * (arr.length - 1))];
  },

  roundDown: function roundDown(num, nearestInt) {
    return Math.floor(num / nearestInt) * nearestInt;
  },

  flatten: function flatten(acc, curr) {
    if (!acc) {
      return [curr];
    }
    if (curr.length) {
  ...
  createDictByProp: function createDictByProp(arr, prop) {
  ...
  bindAll: function bindAll(ctx, fns) {
    fns.forEach(function (d) {
      ctx[d] = ctx[d].bind(ctx);
    });
  },
  dictToArray: function dictToArray(dict) {
    var num = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
var forceManyBody = function() {
  ...
  function force(_) {
    var i, n = nodes.length, tree = quadtree$1(nodes, x$1, y$1).visitAfter(accumulate);
...
var getData = function getData(url) {
  return fetch("src/data/" + url + ".json").then(function (data) {
    return data.json();
  });
};
...
var initialDiversity$1 = 0.3;

var minFolloweeSize = 5;

var cycleDur = 1000;

var width = document.querySelector("#visualization-wrapper").getBoundingClientRect().width;

var height = window.innerHeight;

var messagePassingRecord = {};
var memory = {};
var current = [];
...
var updateMessageReach = function updateMessageReach(id, followersCount, retweet) {
...
var messageState = {
	...
	collectMessages: function collectMessages() {
		for (var i = 0; i < Nodes.length; i++) {
			var messages = Nodes[i].getMessage();
	...
	getMessageReach: function getMessageReach(id) {
		var entry = memory[id];
		return Math.log(entry.actual) / Math.log(Math.max(0.0001, entry.potential));
	},
	getRetweetCount: function getRetweetCount(userA, userB) {
...
var sampleArray$3 = helpers.sampleArray;
var bindAll$1 = helpers.bindAll;
...
var Node = function () {
	...
	createClass(Node, [{
		key: 'getSimilarityOfInitialAndCurrFollowingSets',
		value: function getSimilarityOfInitialAndCurrFollowingSets() {

			var intersectionSet = new Set();
			var unionSet = new Set();
			for (var i = 0; i < this.initialFollowing.length; i++) {
			...
			for (var _i = 0; _i < this.lastFollowing.length; _i++) {
				unionSet.add(this.lastFollowing[_i].id);
			}
		...
		key: 'setDiversity',
		value: function setDiversity() {
			var counts = this.beliefs.reduce(function (acc, curr) {
				acc[curr] = 0;
				return acc;
			}, {});

			for (var i = 0; i < this._following.length; i++) {
				counts[this._following[i].belief]++;
			}

			var sum = values(counts).reduce(function (acc, curr) {
				return acc + curr;
			}, 0);
		...
		key: 'adjustFollowing',
		value: function adjustFollowing() {
			var _this = this;

			this.setDiversity();
			var target = this.diversityOverride === null ? this.desiredDiversity : this.diversityOverride;
			...
			if (this._following.length > minFolloweeSize && this.maintainConnectionsNumber !== false) {
				(function () {
					var choppingBlock = [];
					for (var i = 0; i < _this._following.length; i++) {
						var followee = _this._following[i];
						if (followee.belief === _this.belief) {
							choppingBlock.push(followee.id);
						}
					}

					var unfollowIndex = _this._following.findIndex(function (d) {
						return d.id === sampleArray$3(choppingBlock);
					});

					if (unfollowIndex > -1) {
						if (_this.considerFollowsMutual) {
							for (var _i2 = 0; _i2 < Nodes.length; _i2++) {
								var node = Nodes[_i2];
								if (node.id === _this.following[unfollowIndex].id) {
									var matchingIndex = node.following.map(function (f) {
										return f.id;
									}).indexOf(_this.id);
									if (matchingIndex > -1) {
						...
						_this._following.splice(unfollowIndex, 1);
					}
				})();
			}

			var otherBeliefs = this.beliefs.filter(function (b) {
				return b !== _this.belief;
			});
			var newBelief = sampleArray$3(otherBeliefs);
			var newAction = this.beliefs.findIndex(function (b) {
				return b === newBelief;
			});
			var followingIDs = this._following.map(function (n) {
				return n.id;
			});

			var followingMyFollowees = [];
			for (var i = 0; i < this._following.length; i++) {
				var following = this._following[i].following;
				for (var j = 0; j < following.length; j++) {
					var id = following[j].id;
					if (followingMyFollowees.indexOf(id) === -1) {
						followingMyFollowees.push(id);
					}
					if (this.allowThirdDegree) {
						var nestedFollowing = following[j].following;
						for (var k = 0; k < nestedFollowing.length; k++) {
							var nestedID = nestedFollowing[k].id;
			...
			var availableFollowees = Nodes.filter(function (n) {
				return n.belief === _this.beliefs[newAction] && followingIDs.indexOf(n.id) === -1 && (_this.allowOutsideNetwork ? true : followingMyFollowees.indexOf(n.id) > -1);
			});

			if (availableFollowees.length) {
				var newFollowee = sampleArray$3(availableFollowees);
				this._following.push(newFollowee);

				if (this.considerFollowsMutual) {
					if (newFollowee.following.map(function (n) {
						return n.id;
					}).indexOf(this.id) === -1) {
		...
		key: 'init',
		value: function init() {
			bindAll$1(this, ["cycle", "adjustFollowing"]);
		}
	}, {
		key: 'followedBy',
		set: function set(newFollowedBy) {
			this._followedBy = newFollowedBy;
		},
		get: function get() {
		...
		key: 'lastFollowing',
		set: function set(newLastFollowing) {
			this._lastFollowing = newLastFollowing;
		},
		get: function get() {
			return this._lastFollowing;
		}
	}]);
	return Node;
}();
...
var calculateAssortativity = function calculateAssortativity(Nodes) {
	// unique types
	var types = uniq(Nodes.map(function (n) {
		return n.belief;
	}));
	...
	var total_edges = Nodes.reduce(function (sum, n) {
		sum += n.following.length;return sum;
	}, 0);

	// iterate through the types
	for (var i = 0; i < n_types; i++) {
		var type_i = types[i];
		var nodes_type_i = Nodes.filter(function (n) {
			return n.belief === type_i;
		});
		...
		for (var j = 0; j < n_types; j++) {
			...
			for (var n = 0; n < nodes_type_i.length; n++) {
				...
				var followings_type_j = followings.filter(function (n) {
					return n.belief === type_j;
				});
	...
	var a_i = [];
	for (var i = 0; i < n_types; i++) {
		var a_i_j = e_ij[i].reduce(function (sum, elem) {
			sum += elem;
			return sum;
		}, 0);
		a_i.push(a_i_j);
...
var calculateDistribution$1 = function calculateDistribution$1(Nodes, prop) {
  return Nodes.reduce(function (dict, node) {
...
var create_count_dict = function create_count_dict(Nodes, prop) {
  return Nodes.reduce(function (dict, node) {
...
var newConnectionsCounts = [];
var brokenConnectionsCounts = [];
var assortativity = [];
var initialDiversity = [];
var currentDiversity = [];
var initialFollowerDegrees = [];
var currentFollowerDegrees = [];
var Nodes = void 0;

var initializeNodes = function initializeNodes(seedData, beliefs) {
	Nodes = seedData.map(function (d, i) {
		var belief = d.trumporhillary;
		if (belief === 0) {
			belief = beliefs[0];
...
var cycle$1 = function cycle$1() {
	...
	var diversityDistribution = calculateDistribution$1(Nodes, 'diversity');
	...
	var degreesDistribution = create_count_dict(Nodes, 'followedBy');
...
var saveInitialNodeFollowings = function saveInitialNodeFollowings(node) {
...
var setFollowedBy = function setFollowedBy(node) {
	var toRemove = [],
	    toAdd = [];
	...
	for (var i = 0; i < node.following.length; i++) {
		...
		var thisNode = node.following[i];
		for (var j = 0; j < node.lastFollowing.length; j++) {
	...
	for (var _i2 = 0; _i2 < node.lastFollowing.length; _i2++) {
		var isFollowing = false;
		var _thisNode = node.lastFollowing[_i2];
		for (var _j = 0; _j < node.following.length; _j++) {
			if (node.following[_j].id === _thisNode.id) {
				isFollowing = true;
				break;
			}
		}
	...
	var _loop = function _loop(_i3) {
		var match = Nodes.find(function (d) {
			return d.id === toRemove[_i3].id;
		});
		var index = match.followedBy.indexOf(node.id);
	...
	var _loop2 = function _loop2(_i4) {
		var match = Nodes.find(function (d) {
			return d.id === toAdd[_i4].id;
		});
		match.followedBy = match.followedBy.concat(node.id);
	};

	for (var _i4 = 0; _i4 < toAdd.length; _i4++) {
		_loop2(_i4);
	}
...
setTimeout(function () {
	mediator.subscribe("selectDataset", function () {
		Nodes = [];
		assortativity = [];
		initialDiversity = [];
		currentDiversity = [];
...
mediator.subscribe("editFriends", function (d) {
...
mediator.subscribe("mutualFollows", function (d) {
	Nodes.forEach(function (n) {
		n.considerFollowsMutual = d;
	});
});

mediator.subscribe("whetherToUnfollow", function (d) {
	Nodes.forEach(function (n) {
...
var initialDataset = 'downTerrorism';
var currentDataset = void 0;
...
var updatePickerVisibility = function updatePickerVisibility() {
	if (pickerOpen) {
...
var selectDataset = function selectDataset(dataset) {
	var activeNode = document.querySelector("[data-dataset=" + dataset + "]");

	Array.prototype.forEach.call(document.querySelectorAll("#dataset-picker .dropdown .option"), function (node) {
...
mediator.subscribe("restart", function () {
	selectDataset(currentDataset);
});

setTimeout(function () {
	// timeout - wait for index.js to subscribe
...
var Visualization = function () {
  function Visualization(svg, width, height, testName) {
    ...
    this.xScale = linear().domain([0, Math.round(width / 40)]).range([0, width]);
  }
  ...
  createClass(Visualization, [{
    ...
    value: function setup() {
      ...
      this.xAxis.attr("x1", 0).attr("x2", this.width).attr("y1", this.height * 0.5).attr("y2", this.height * 0.5);

      this.yAxisMax = this.svg.append("text").attr("class", "y-max-label").attr("x", 0).attr("y", 0);

      this.yAxisMin = this.svg.append("text").attr("class", "y-min-label").attr("x", 0).attr("y", this.height);

      this.xAxisLabelsGroup = this.svg.append("g").attr('class', 'x-axis-labels');

      this.convergenceRect = this.svg.insert("rect", ':first-child').attr("class", "convergence-marker").attr("y", this.height / 2 + rectSize).attr("width", rectSize).attr("height", 1);
    }
  }, {
    key: "update",
    value: function update(arr) {
      ...
      var xAxisLabels = this.xAxisLabelsGroup.selectAll("text").data(arr.slice(0, arr.length - 1));

      xAxisLabels.exit().remove();

      xAxisLabels.enter().append("text").text(function (d, i) {
        return i + 1;
      }).attr("x", function (d, i) {
        return _this.xScale(i + 1);
      ...
      this.convergenceRect.attr("x", function () {
        return _this.xScale(arr.length - 1) - rectSize / 2;
      });
    }
  }, {
    key: "clear",
...
var ConvergenceTimeseries = function (_VisualizationBase) {
	inherits(ConvergenceTimeseries, _VisualizationBase);

	function ConvergenceTimeseries(svg, width, height, testName) {
		classCallCheck(this, ConvergenceTimeseries);

		var _this = possibleConstructorReturn(this, (ConvergenceTimeseries.__proto__ || Object.getPrototypeOf(ConvergenceTimeseries)).call(this, svg, width, height, testName));

		_this.yScale = linear().domain([0, 100]) // this should be some reasonable estimate, and it needs to update as the chart draws itself
		.range([0, height / 2]).clamp(true);

		_this.addedLineGenerator = line().x(function (d, i) {
			return _this.xScale(i);
		}).y(function (d) {
			return _this.height / 2 - _this.yScale(d);
		});

		_this.removedLineGenerator = line().x(function (d, i) {
			return _this.xScale(i);
		}).y(function (d) {
			return _this.height / 2 + _this.yScale(d);
		});
	...
	createClass(ConvergenceTimeseries, [{
		key: 'setup',
		value: function setup() {
			get$2(ConvergenceTimeseries.prototype.__proto__ || Object.getPrototypeOf(ConvergenceTimeseries.prototype), 'setup', this).call(this);
		...
		value: function update() {
			var maxVal = Math.max(newConnectionsCounts[0], brokenConnectionsCounts[0]);

			if (isNaN(maxVal)) return;

			get$2(ConvergenceTimeseries.prototype.__proto__ || Object.getPrototypeOf(ConvergenceTimeseries.prototype), 'update', this).call(this, newConnectionsCounts);
			...
			this.addedPath.data([newConnectionsCounts]).attr("d", this.addedLineGenerator);

			this.removedPath.data([brokenConnectionsCounts]).attr("d", this.removedLineGenerator);
		}
	}]);
	return ConvergenceTimeseries;
}(Visualization);

var AssortativityChart = function (_VisualizationBase) {
	inherits(AssortativityChart, _VisualizationBase);

	function AssortativityChart(svg, width, height, testName) {
		classCallCheck(this, AssortativityChart);

		var _this = possibleConstructorReturn(this, (AssortativityChart.__proto__ || Object.getPrototypeOf(AssortativityChart)).call(this, svg, width, height, testName));

		_this.yScale = linear().domain([-1, 1]) // this should be some reasonable estimate, and it needs to update as the chart draws itself
		.range([0, height]).clamp(true);
	...
	createClass(AssortativityChart, [{
		key: 'setup',
		value: function setup() {
			get$2(AssortativityChart.prototype.__proto__ || Object.getPrototypeOf(AssortativityChart.prototype), 'setup', this).call(this);
		...
		key: 'update',
		value: function update(Nodes$$1) {
			get$2(AssortativityChart.prototype.__proto__ || Object.getPrototypeOf(AssortativityChart.prototype), 'update', this).call(this, assortativity);

			this.addedPath.data([assortativity]).attr("d", this.addedLineGenerator);
		}
	}]);
	return AssortativityChart;
}(Visualization);

var dictToArray$1 = helpers.dictToArray;

var DiversityHistogram = function (_VisualizationBase) {
  inherits(DiversityHistogram, _VisualizationBase);

  function DiversityHistogram(svg, width, height, testName) {
    classCallCheck(this, DiversityHistogram);

    var _this = possibleConstructorReturn(this, (DiversityHistogram.__proto__ || Object.getPrototypeOf(DiversityHistogram)).call(this, svg, width, height, testName));

    _this.xIncrements = 10;

    _this.yScale = linear().domain([0, 100]) // should dynamically update
    .range([0, height]).clamp(true);

    _this.xScale = linear().domain([0, 1]).range([0, width]).clamp(true);

    _this.lineGenerator = line().x(function (d) {
      return _this.xScale(isNaN(d[0]) ? 0 : +d[0]);
    }).y(function (d) {
      return _this.height - _this.yScale(d[1]);
    });
  ...
  createClass(DiversityHistogram, [{
    key: 'setup',
    value: function setup() {
      var _this2 = this;

      get$2(DiversityHistogram.prototype.__proto__ || Object.getPrototypeOf(DiversityHistogram.prototype), 'setup', this).call(this);

      this.xAxis.attr("y1", this.height).attr("y2", this.height);

      var xAxisLabels = this.xAxisLabelsGroup.selectAll("text").data(range(this.xIncrements).map(function (d, i) {
        return i * (1 / _this2.xIncrements);
      }).concat(1));

      xAxisLabels.enter().append("text").text(function (d) {
        return d.toFixed(1);
      }).attr("x", this.xScale).attr("y", this.height + 10);
      ...
      this.diversityLine = this.svg.append("line").attr("class", "diversity-line").attr("y1", 0).attr("y2", this.height).attr("x1", this.diversity * this.width).attr("x2", this.diversity * this.width);
    }
  }, {
    key: 'update',
    value: function update(Nodes$$1) {
      ...
      var initialDiversityArray = dictToArray$1(initialDiversity);
      var currentDiversityArray = dictToArray$1(currentDiversity);
      var yMax = Math.max.apply(Math, toConsumableArray(initialDiversityArray.map(function (d) {
        return d[1];
      }).concat(currentDiversityArray.map(function (d) {
        return d[1];
      }))));
      ...
      this.initialDiversityPath.data([initialDiversityArray]).attr("d", function (d) {
        return _this3.lineGenerator(d) + ' L' + _this3.width + ',' + _this3.height + ' L0,' + _this3.height + 'Z';
      });

      this.currentDiversityPath.data([currentDiversityArray]).attr("d", this.lineGenerator);
    }
  }]);
  return DiversityHistogram;
}(Visualization);

var BeliefBarChart = function (_VisualizationBase2) {
  inherits(BeliefBarChart, _VisualizationBase2);

  function BeliefBarChart(svg, width, height, testName) {
    classCallCheck(this, BeliefBarChart);

    var _this4 = possibleConstructorReturn(this, (BeliefBarChart.__proto__ || Object.getPrototypeOf(BeliefBarChart)).call(this, svg, width, height, testName));

    _this4.history = [];
    return _this4;
  }

  createClass(BeliefBarChart, [{
    key: 'update',
    value: function update(Nodes$$1) {
      var diversity_counts = dictToArray$1(calculateDistribution(Nodes$$1, 'belief'));
  ...
  return BeliefBarChart;
}(Visualization);

var dictToArray$2 = helpers.dictToArray;

var FollowerDegrees = function (_VisualizationBase) {
  inherits(FollowerDegrees, _VisualizationBase);

  function FollowerDegrees(svg, width, height, testName) {
    classCallCheck(this, FollowerDegrees);

    var _this = possibleConstructorReturn(this, (FollowerDegrees.__proto__ || Object.getPrototypeOf(FollowerDegrees)).call(this, svg, width, height, testName));
  ...
  createClass(FollowerDegrees, [{
    key: 'setup',
    value: function setup() {
      get$2(FollowerDegrees.prototype.__proto__ || Object.getPrototypeOf(FollowerDegrees.prototype), 'setup', this).call(this);
    ...
    value: function update(Nodes$$1) {
      ...
      var initialFollowerDegreesArray = dictToArray$2(initialFollowerDegrees);
      var currentFollowerDegreesArray = dictToArray$2(currentFollowerDegrees);

      var yMax = Math.max.apply(Math, toConsumableArray(initialFollowerDegreesArray.map(function (d) {
        return d[1];
      }).concat(currentFollowerDegreesArray.map(function (d) {
        return d[1];
      }))));

      var xMax = Math.max.apply(Math, toConsumableArray(initialFollowerDegreesArray.map(function (d) {
        return d[0];
      }).concat(currentFollowerDegreesArray.map(function (d) {
        return d[0];
      }))));
      ...
      this.initialFollowerDegreesPath.data([initialFollowerDegreesArray]).attr("d", function (d) {
        return _this2.lineGenerator(d) + ' L' + _this2.xScale(initialFollowerDegreesArray[initialFollowerDegreesArray.length - 1][0]) + ',' + _this2.height + ' L0,' + _this2.height + 'Z';
      });

      this.currentFollowerDegreesPath.data([currentFollowerDegreesArray]).attr("d", this.lineGenerator);

      var xAxisLabels = this.xAxisLabelsGroup.selectAll("text").data(currentFollowerDegreesArray);

      xAxisLabels.exit().remove();

      xAxisLabels.enter().append("text");

      xAxisLabels.text(function (d) {
        return d[0];
      }).attr("x", function (d) {
...
var charts = { ConvergenceTimeseries: ConvergenceTimeseries, AssortativityChart: AssortativityChart, DiversityHistogram: DiversityHistogram, FollowerDegrees: FollowerDegrees };

var updateSID = null;
var activeChart = 'ConvergenceTimeseries';

var visDOM = select("#visualization");

var svg = visDOM.select("svg");

var svgWidth = svg.node().parentNode.getBoundingClientRect().width;
var svgHeight = Math.min(height / 6, 200);
...
var beliefs$1 = ['conservative', 'neutral', 'liberal'];

mediator.subscribe("data-initialized", function (edges) {
	// update static network properties
	var beliefDistribution = calculateDistribution$1(Nodes, 'belief');

	beliefs$1.forEach(function (belief) {
		document.querySelector('.' + belief).textContent = '0';
	});
	Object.keys(beliefDistribution).forEach(function (belief) {
...
mediator.subscribe("selectDataset", function (dataset) {
...
mediator.subscribe("converged", function () {
	window.clearInterval(updateSID);
	svg.attr("data-converged", true);
});

var buildDropdown = function buildDropdown() {
	Object.keys(charts).forEach(function (d) {
...
var selectOption = function selectOption(d) {
	console.log('selecting options', d);
	activeChart = d;
	document.querySelector(".select-visualization .current").textContent = d;

	Array.prototype.forEach.call(document.querySelectorAll(".select-visualization .option"), function (el) {
...
var currentDiversity$1 = initialDiversity$1;
...
document.addEventListener("mousemove", function (e) {
	...
	var left = Math.max(0, Math.min(sliderWidth, e.clientX - sliderLeft));
	updateDiversity$1(left);
});

mediator.subscribe("data-initialized", function () {
	updateDiversity$1(currentDiversity$1 * sliderWidth);
});
...
mediator.subscribe("data-initialized", play$1);

mediator.subscribe("stopped", stop$1);

var current$1 = 'second-degrees';

var options = ['second-degrees', 'third-degrees', 'edit-friends-friends'].reduce(function (acc, curr) {
	acc[curr] = {
...
var update$1 = function update$1(status) {
	current$1 = status;
...
document.addEventListener("click", function (e) {
	var option = e.target.closest(".degrees-option");
	if (option) {
		e.stopPropagation();
		update$1(option.getAttribute("id"));
	}
});
...
var update$2 = function update$2(status) {
	checked = status;
...
var update$3 = function update$3(status) {
	checked$1 = status;
	document.querySelector("#whether-to-unfollow").setAttribute("data-checked", checked$1);
	mediator.publish("whetherToUnfollow", checked$1);
};
...
var roundDown$1 = helpers.roundDown;
var decodeFloat$1 = helpers.decodeFloat;
...
var network_stats = document.querySelector("#network_stats");
var popoverID = popoverElement.querySelector(".node_id");
var popoverDiversityOverride = popoverElement.querySelector('.node_diversity_override .count');
var popoverDiversity = popoverElement.querySelector('.node_diversity .count');
var popoverFollowees = popoverElement.querySelector('.node_followees .count');
var popoverFollowers = popoverElement.querySelector('.node_followers .count');
var popoverInitialConnections = popoverElement.querySelector('.node_initial_connections .count');
var quadtree$$1 = quadtree$1();
var renderer = new THREE.WebGLRenderer({ alpha: true, canvas: document.querySelector("#webgl-canvas") });
var scene = new THREE.Scene();
var camera = new THREE.OrthographicCamera(width / -2, width / 2, height / 2, height / -2, 1, 10000);
var nodePositions = void 0;
var nodeSizesColors = void 0;
var edgeGeometry = void 0;
var nodeGeometry = void 0;
var edgeColorsStartTimes = void 0;
var edgeColorsStartTimesBuffer = void 0;
var edgeVertices = void 0;
var lastOccupiedEdgeVertexIndex = void 0;
var nodePositionBuffer = void 0;
var edgeVerticesBuffer = void 0;
var nodeSizesColorsBuffer = void 0;
var force = forceSimulation();
var emptyNode = new THREE.Vector2();
var links = void 0;
var nodeData = void 0;
var edgeData = void 0;
var globalJaccardInitialCurrFollowing = 0;
var cycleSID = null;
var updateLinksSID = null;
var updateLinksNodeIndex = 0;
var maxFollowedByLength = 0;
var minFollowedByLength = Infinity;
var nodeSizeScale = linear().range([5, 25]).clamp(true);
var peakTime = 250.0;
var totalTime = 350.0;
var canvasLeft = 0;
var canvasTop = 0;
var match = void 0;
var lineSegments = void 0;
var points = void 0;
var nodeMaterial = void 0;
var edgeMaterial = void 0;
...
var updateMinMaxFollowedBy = function updateMinMaxFollowedBy(length) {
...
var loop = function loop() {
  ...
  var shouldUpdate = Math.random() < 0.5; // perf
  ...
  if (animating) {
    ...
    for (var i = 0; i < Nodes.length; i++) {
      ...
      if (n.following.length) {
        for (var j = 0; j < n.following.length; j++) {
          var target = void 0;
  ...
  for (var _i = 0; _i < links.length; _i++) {
    ...
    if (_i < lastOccupiedEdgeVertexIndex) {
      edgeVertices[_i * 2 * 3] = source.x - width / 2;
      edgeVertices[_i * 2 * 3 + 1] = -(source.y - height / 2);
      edgeVertices[_i * 2 * 3 + 3] = _target.x - width / 2;
      edgeVertices[_i * 2 * 3 + 4] = -(_target.y - height / 2);
    }

    if (window.activeNode) {
      if (source.id === window.activeNode.id) {
        edgeColorsStartTimes[_i * 2 * 2 + 1] = d - peakTime;
        edgeColorsStartTimes[_i * 2 * 2 + 3] = d - peakTime;
      }
    } else {
      if (source.newlyFollowing && source.newlyFollowing.length !== source.following.length) {
        var newlyFollowingIDs = source.newlyFollowing.map(function (d) {
          return d.id;
        });

        if (newlyFollowingIDs.indexOf(_target.id) > -1) {
          if (d - edgeColorsStartTimes[_i * 2 * 2 + 1] > cycleDur && d - edgeColorsStartTimes[_i * 2 * 2 + 3] > cycleDur) {
            // source
            edgeColorsStartTimes[_i * 2 * 2 + 1] = d - peakTime;
            // target
            edgeColorsStartTimes[_i * 2 * 2 + 3] = d;
          }
        }
      }
    }
  ...
  var jointJaccardNum = 0;
  var jointJaccardDenom = 0;
  Nodes.forEach(function (n) {
    var jaccardValues = n.getSimilarityOfInitialAndCurrFollowingSets();
    jointJaccardNum += jaccardValues[0];
    jointJaccardDenom += jaccardValues[1];
  });

  if (lastOccupiedEdgeVertexIndex > links.length) {
    for (var _i2 = links.length; _i2 < lastOccupiedEdgeVertexIndex; _i2++) {
      edgeVertices[_i2 * 2 * 3] = 0;
      edgeVertices[_i2 * 2 * 3 + 1] = 0;
      edgeVertices[_i2 * 2 * 3 + 3] = 0;
      edgeVertices[_i2 * 2 * 3 + 4] = 0;
    }
  }
  ...
  if (animating) {
    var diff = d - lastCycleTime;
    var targetIndex = Math.max(0, Math.min(Math.round(diff / cycleDur * Nodes.length), Nodes.length));

    if (targetIndex < updateLinksNodeIndex) {
      updateLinksNodeIndex = 0;
    } // wrap around

    for (var _i3 = updateLinksNodeIndex; _i3 < targetIndex; _i3++) {
      var node = Nodes[_i3];
  ...
  if (shouldUpdate) {
    ...
    quadtree$$1 = quadtree$1().extent([[-1, -1], [width, height]]);
  ...
  for (var _i4 = 0; _i4 < Nodes.length; _i4++) {
    var _node = Nodes[_i4];
    var opacity = 254;
    if (window.activeNode && _node.id !== window.activeNode.id) {
      var mutualFollow = _node.following.map(function (f) {
        return f.id;
      }).indexOf(window.activeNode.id) > -1 && window.activeNode.following.map(function (f) {
        return f.id;
      }).indexOf(_node.id) > -1;
      if (!mutualFollow) {
        opacity = 100;
    ...
    nodePositions[_i4 * 2] = _node.x - width / 2;
    nodePositions[_i4 * 2 + 1] = -(_node.y - height / 2);
    nodeSizesColors[_i4 * 2] = nodeSizeScale(_node.followedBy.length);
    if (_node.belief === "conservative") {
      // red
      nodeSizesColors[_i4 * 2 + 1] = decodeFloat$1(254, 25, 83, opacity);
    } else if (_node.belief === "liberal") {
      // blue
      nodeSizesColors[_i4 * 2 + 1] = decodeFloat$1(0, 190, 254, opacity);
    } else {
      // white
      nodeSizesColors[_i4 * 2 + 1] = decodeFloat$1(254, 254, 254, opacity);
    }

    if (window.activeNode && _node.id === window.activeNode.id) {
      halo.style.transform = 'translate3d(' + (canvasLeft + _node.x - 6) + 'px, ' + (canvasTop + _node.y - 6) + 'px, 0)';
    }

    if (shouldUpdate) {
      quadtree$$1.add([_node.x, _node.y, _node]);
      // initNetworkConnectivity(Nodes)
      updateMinMaxFollowedBy(_node.followedBy.length);
    }
  }
...
var initialize = function initialize() {
  ...
  force.nodes(Nodes).force("link", forceLink().id(function (d) {
    return d.id;
  })).force("charge", forceManyBody().strength(-10).distanceMax(200)).force("center", forceCenter(width / 2, height / 2)).force("vertical", forceY().strength(0.01)).force("horizontal", forceX().strength(0.01)).velocityDecay(0.6);
  ...
  Nodes.forEach(function (n) {
    return saveInitialNodeFollowings(n);
  });
  // initFlot(Nodes[20]);
...
var revealHalo = function revealHalo(x, y) {
  halo.style.transform = 'translate3d(' + (x - 6) + 'px, ' + (y - 6) + 'px, 0)';
  halo.classList.add("active");
};

var removeHalo = function removeHalo() {
...
document.addEventListener("mousemove", function (e) {
  ...
  match = quadtree$$1.find(e.pageX - canvasLeft, e.pageY - canvasTop, 3);
  if (match) {
    var node = match[2];
    var diversity = node.diversity;
    var target = node.diversityOverride === null ? node.desiredDiversity : node.diversityOverride;
    var jaccardValues = node.getSimilarityOfInitialAndCurrFollowingSets();
    var jaccardChange = (1 - parseFloat(jaccardValues[0]) / jaccardValues[1]) * 100;
...
mediator.subscribe("selectDataset", function (dataset) {
  ...
  Promise.all([dataset.nodes, dataset.edges].map(getData)).then(function (data) {
    ...
    nodeData.splice(roundDown$1(nodeData.length, 3)); // nodes length must be multiple of 3

    edgeData = shuffle(data[1].filter(function (d) {
      return [+d.source, +d.target].every(function (id) {
        return nodeData.find(function (n) {
          return n.node_id === id;
        });
      });
    }));

    edgeData.splice(roundDown$1(edgeData.length, 3));
